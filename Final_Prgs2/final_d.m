...
xlabel('Iteration Step');
ylabel('P value');
title('Convergence of EM algorithm for P_i values (nonparametric)');
saveas(gcf,'gp_em_converge_nonpara.png');

[0.5 0.3 0.2; 300 180 120;-3 0 2; 3 0.5 1]
[cp(250,1) cp(250,2) cp(250,3);length(d1) length(d2) length(d3);mean(d1) mean(d2) mean(d3); var(d1) var(d2) var(d3)]
[LL1(MAXIT) LL2(MAXIT) LL3(MAXIT); length(s1) length(s2) length(s3);mean(s1) mean(s2) mean(s3); var(s1) var(s2) var(s3)]
